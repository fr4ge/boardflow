...
Commands exposed by the project and its dependencies (like `boardflow` or `pytest`) can be run via `uv` without having to activate the python virtual environment in the shell, for example:
